...
const queries = {
  ...
  sendNotifications: `
    ...
    `,
  getNotificationsList: `SELECT ${table_name}.*, u.first_name, u.last_name, f.flat_number FROM ${table_name} join users u on ${table_name}.user_id = u.user_id join tenancies t on ${table_name}.user_id = t.user_id join flats f on t.flats_id = f.flats_id WHERE ${table_name}.owner_id = ? AND ${table_name}.created_by = ${table_name}.owner_id`,
